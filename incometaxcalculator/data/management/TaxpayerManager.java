...
public class TaxpayerManager {
  ...
  public void createTaxpayer(String fullname, int taxRegistrationNumber, String status,
    ...
    TaxpayerFactory taxpayerFactory = new TaxpayerFactory(fullname, taxRegistrationNumber, income);
    Taxpayer taxpayer = taxpayerFactory.createTaxpayer(status);
  ...
  private void updateFiles(int taxRegistrationNumber) throws IOException {
    ...
    if (new File(taxRegistrationNumber + "_INFO.xml").exists()) {
      FileWriterFactory.createFileWriter("xmlInfo").generateFile(taxRegistrationNumber);
    } else {
      FileWriterFactory.createFileWriter("txtInfo").generateFile(taxRegistrationNumber);
      return;
    }
